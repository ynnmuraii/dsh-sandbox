...
const SHARED_CONTEXT_STUB = `# Shared context snapshot
> context version: not-synced

> This snapshot is a placeholder written by \`lab new\`. It is regenerated by
> \`lab sync-context\` once Task 6 (context sync) is wired up.
`
...
export async function createPlugin(opts: { root: string; name: string }): Promise<string> {
  ...
  // Stub shared-context snapshot so create and Task 6's sync stay consistent.
